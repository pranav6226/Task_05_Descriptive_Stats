...
# Calculate overs faced by each team in power-play
powerplay_overs = powerplay_data.groupby('batting_team')['over'].nunique().reset_index()
powerplay_overs = powerplay_overs.rename(columns={'batting_team': 'team', 'over': 'overs_faced'})

# Merge runs and overs
powerplay_stats = powerplay_runs.merge(powerplay_overs, on='team', how='left')
...
for idx, row in powerplay_stats.iterrows():
    ...
    overs = int(row['overs_faced'])
    rpo = row['runs_per_over']
    
    print(f"{team}: {runs} runs in {overs} overs ({rpo:.2f} runs per over)")
